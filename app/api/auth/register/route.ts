...
export async function POST(req: Request) {
  ...
  try {
    ...
    // se já tiver profile, não cria de novo
    ...
    if (existingProfile?.tenant_id) {
      console.log("[REGISTER] user already has tenant:", existingProfile.tenant_id);
      return NextResponse.json({
        ok: true,
        reused: true,
        tenantId: String(existingProfile.tenant_id),
    ...
    if (tErr || !t?.id) {
      ...
      return jsonError("Falha ao criar tenant", 500, { code: tErr?.code, message: tErr?.message });
    }
    ...
    if (pErr) {
      ...
      return jsonError("Falha ao criar profile", 500, { code: pErr.code, message: pErr.message });
    }
    ...
    return jsonError("Erro inesperado no cadastro", 500, { message: String(err?.message || err) });
  }
}
